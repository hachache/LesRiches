...
export function BillionairesList() {
  ...
  const parsedSalary = Number(monthlySalary.replace(",", "."));
      ...
      <div className="paper-panel grid gap-5 rounded-none border-black/40 p-4">
        ...
        <div className="grid gap-3 border-t border-black/10 pt-4 md:grid-cols-[280px_1fr] md:items-end">
          ...
          <p className="text-sm leading-6 text-[var(--muted)]">
            Les cartes comparent chaque fortune à {formatCurrencyEUR(usableSalary)} net par mois, puis à une
            hypothèse de 20% d'épargne. C'est un repère d'échelle, pas un jugement politique.
          </p>
        </div>
      </div>
      <div className="grid gap-4">
        {filtered.map((person) => (
          <BillionaireCard key={person.name} billionaire={person} monthlySalary={usableSalary} />
        ))}
      </div>
    </section>
  );
